...
export default function Sidebar() {
  ...
  return (
    <>
      <div className="MY AIO-sidebar fixed inset-y-0 left-0 z-50">
        <div className="MY AIO-logo">
          <Link to="/dashboard">
            <img src={logo} alt="MYAIO" />
          </Link>
        </div>

        <nav className="MY AIO-nav">
          {navItems
              ...
              if (!hasChildren) {
                return (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      isActive ? "MY AIO-nav-item active" : "text-black MY AIO-nav-item"
                    }
                  >
              ...
              return (
                <div key={item.label}>
                  <button
                    ...
                    className={`MY AIO-nav-item w-100 text-start d-flex align-items-center justify-content-between ${
                  ...
                  >
                    {item.children?.map((child) => (
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          isActive
                            ? "MY AIO-nav-item MY AIO-submenu-item active ps-5"
                            : "MY AIO-nav-item text-black MY AIO-submenu-item ps-5"
                        }
                      >
          ...
          <button
            onClick={handleShow}
            className="MY AIO-logout d-flex align-items-center gap-3 text-danger hover-bg-danger-subtle"
            style={{
              width: "100%",
              padding: "12px 16px",
              border: "none",
        ...
        <div className="MY AIO-user">
          <div className="MY AIO-user-info">
            <div className="MY AIO-avatar">{user?.name?.[0] || "U"}</div>
            <div>
              <div
                className="MY AIO-user-name"
                style={{ textTransform: "capitalize" }}
              >
                {user?.name || "Agent"}
              </div>
              {user?.role == "ADMIN" ? null : (
                <div className="MY AIO-user-status">
